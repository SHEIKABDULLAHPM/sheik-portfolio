          ...
          <div className="relative flex items-center justify-center">
            ...
            <div className="relative w-full max-w-[280px] rounded-[32px] border border-white/10 bg-slate-900/70 p-3 shadow-2xl shadow-indigo-500/20 backdrop-blur sm:max-w-[320px] sm:p-4">
              <div className="relative mx-auto aspect-[3/4] w-full overflow-hidden rounded-[22px] border border-white/10">
                <img src={withBasePath(profile.image)} alt={profile.name} className="h-full w-full object-cover" loading="lazy" />
            ...
            <div className="mt-3 flex flex-wrap gap-3">
              {socialLinks.map((link) => (
                ...
                >
                    {link.image ? (
                    <img
                      src={withBasePath(link.image)}
                      alt={`${link.label} logo`}
                      className="h-7 w-7 object-contain sm:h-8 sm:w-8"
                      loading="lazy"
                    />
                  ) : (
                    <span className="text-sm font-semibold text-indigo-200 sm:text-base">{getFallbackBadge(link.label)}</span>
                  )}
